...
\begin{enumerate}[label=\textbf{\arabic*.}]
  ...
  in $O(mn)$ time and $O(m)$ space but the problem is that we can't backtrack and find the path itself, as we are deleting the old rows. But this can help in actually finding the $i$ which minimizes $f(i,n/2) + g(i,n/2)$. We can do our $O(m)$
